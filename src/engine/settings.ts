...
export const DEFAULT_CONFIG: BotConfig = {
  ...
  flat_kill_seconds: 30,
  flat_kill_multiple: 1.05,
  dead_mcap: 3000,
  hard_death_from_fill_pct: 0.55,
  flatline_mcap_max: 4000,
  flatline_seconds: 25,
...
export const CONFIG_META: {
  ...
  {
    key: "live_any_socials",
    label: "Live: any coin with socials",
    group: "mode",
    kind: "bool",
    hint: "Live only. Trade any create that passes the other rules (socials, skip-mcap, risk limits), not just trusted DEV wallets. Ignored in dry-run.",
  },
  ...
  { key: "sell_aggressiveness", label: "Sell aggressiveness", group: "size", kind: "number", hint: "0–100 dial: 0 = first sell at 2.1× bank 20%, 100 = scalp everything at 1.25×. Dead tape sells harder; ripping tape holds." },
  ...
  { key: "flat_kill_seconds", label: "Flat/dump kill (s)", group: "phases", kind: "number", hint: "If it never printed or is already red, flatten after this many seconds." },
  { key: "flat_kill_multiple", label: "Flat kill if under (× fill)", group: "phases", kind: "number", hint: "Must print above this by t+flat_kill or we sell 100%." },
  ...
  { key: "flatline_mcap_max", label: "Flatline mcap max", group: "phases", kind: "number", hint: "Stuck between dead mcap and this for flatline seconds → sell 100%." },
  { key: "flatline_seconds", label: "Flatline seconds", group: "phases", kind: "number", hint: "Seconds stuck in the dead band with no escape before selling 100%." },
 ...
 * - first bank multiple: 2.1× (dial 0) → 1.25× (dial 100), quantized to 0.05 steps
...
export function bankParams(cfg: BotConfig): { firstMult: number; bankFrac: number } {
  ...
  const raw = 2.1 - (a / 100) * 0.85;
  const firstMult = Math.round(raw / 0.05) * 0.05;
